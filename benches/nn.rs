...
use ml2::nn::{distance::*, CommonNearestNeighbour, NearestNeighbour};
...
fn nn_build_bench(c: &mut Criterion) {
	...
	let n_features = 3;
	let algorithms = &[(CommonNearestNeighbour::KdTree, "kdtree"), (CommonNearestNeighbour::BallTree, "balltree")];

	for &n_points in &[1000, 5000, 10000] {
		let rng = &mut rng;
		let points_arr = Array2::random_using((n_points, n_features), Uniform::new(-500., 500.), rng);

		for (alg, name) in algorithms {
			benchmark.bench_with_input(BenchmarkId::new(*name, format!("{}", n_points)), &points_arr, |bencher, points_arr| {
...
fn k_nearest_bench(c: &mut Criterion) {
	...
	for &(n_points, k) in &[(10000, 10), (50000, 100), (50000, 1000)] {
		...
		for (alg, name) in algorithms {
			...
			benchmark.bench_with_input(BenchmarkId::new(*name, format!("{}-{}", n_points, k)), &k, |bencher, &k| {
				bencher.iter(|| {
...
fn within_range_bench(c: &mut Criterion) {
	...
	for &(n_points, range) in &[(50000, 10.0), (50000, 20.0)] {
		...
		for (alg, name) in algorithms {
			let nn = alg.from_batch(&points, L2Dist).unwrap();
			benchmark.bench_with_input(BenchmarkId::new(*name, format!("{}-{}", n_points, range)), &range, |bencher, &range| {
				bencher.iter(|| {
