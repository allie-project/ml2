...
//!
//! The purpose of this crate is to faciliate dataset loading and make it as simple as possible. Loaded datasets are
//! returned as a [linfa::Dataset] structure with named features.
//!
//! Additionally, this crate provides utility functions to randomly generate test datasets.
//!
//! ## Using a dataset
//!
//! To use one of the provided datasets in your project add the `linfa-datasets` crate to your `Cargo.toml` and enable
...
#[cfg(any(feature = "iris", feature = "diabetes", feature = "winequality", feature = "linnerud"))]
...
pub fn iris() -> Dataset<f64, usize, Ix1> {
	...
	let (data, targets) = (array.slice(s![.., 0..4]).to_owned(), array.column(4).to_owned());

	let feature_names = vec!["sepal length", "sepal width", "petal length", "petal width"];

	Dataset::new(data, targets).map_targets(|x| *x as usize).with_feature_names(feature_names)
}
...
pub fn winequality() -> Dataset<f64, usize, Ix1> {
	...
	let (data, targets) = (array.slice(s![.., 0..11]).to_owned(), array.column(11).to_owned());
...
mod tests {
	...
	fn test_iris() {
		...
		assert_eq!(ds.feature_names(), &["sepal length", "sepal width", "petal length", "petal width"]);

		// check label frequency
		assert_abs_diff_eq!(ds.label_frequencies().into_iter().map(|b| b.1).collect::<Array1<_>>(), array![50., 50., 50.]);
		...
		assert_abs_diff_eq!(mean_features, array![5.84, 3.05, 3.75, 1.20], epsilon = 0.01);
	}
	...
	fn test_winequality() {
		...
		// check that we have the right amount of data
		assert_eq!((ds.nsamples(), ds.nfeatures(), ds.ntargets()), (1599, 11, 1));
		...
		// check label frequency
		let compare_to = vec![(5, 681.0), (7, 199.0), (6, 638.0), (8, 18.0), (3, 10.0), (4, 53.0)];
