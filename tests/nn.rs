...
use ml2::nn::{distance::*, CommonNearestNeighbour, LinearSearch, NearestNeighbour};
...
fn sort_by_dist<'a>(mut vec: Vec<(ArrayView1<'a, f64>, usize)>, pt: ArrayView1<f64>) -> Vec<(ArrayView1<'a, f64>, usize)> {
	vec.sort_by_key(|v| NoisyFloat::<_, FiniteChecker>::new(v.0.sq_l2_dist(&pt).unwrap()));
	vec
}

fn assert_query(output: Vec<(ArrayView1<f64>, usize)>, input_data: &Array2<f64>, exp_pos: Vec<usize>) {
	let (pts, pos): (Vec<_>, Vec<_>) = output.into_iter().unzip();
	assert_eq!(pos, exp_pos);
	assert_abs_diff_eq!(stack(Axis(0), &pts).unwrap(), input_data.select(Axis(0), &exp_pos));
}
...
fn nn_test_error(builder: &CommonNearestNeighbour) {
	...
	assert!(builder.from_batch_with_leaf_size(&points, 0, L2Dist).is_err());
	let nn = builder.from_batch(&points, L2Dist).unwrap();
...
fn nn_test_degenerate(builder: &CommonNearestNeighbour) {
	...
	let out = nn
		...
		.map(|(p, _)| p.reborrow())
		.collect::<Vec<_>>();
	assert_abs_diff_eq!(stack(Axis(0), &out).unwrap(), arr2(&[[0.0, 2.0], [0.0, 2.0]]));
	...
	let out = nn
		...
		.map(|(p, _)| p.reborrow())
		.collect::<Vec<_>>();
	assert_abs_diff_eq!(stack(Axis(0), &out).unwrap(), arr2(&[[0.0, 2.0], [0.0, 2.0], [0.0, 2.0]]));
	...
	let out = nn
		...
		.map(|(p, _)| p.reborrow())
		.collect::<Vec<_>>();
	assert_eq!(out, Vec::<ArrayView1<f64>>::new());
	let out = nn
...
fn assert_eq_queries(out1: Vec<(ArrayView1<f64>, usize)>, out2: Vec<(ArrayView1<f64>, usize)>) {
	...
	assert_eq!(pos1, pos2);
	assert_abs_diff_eq!(stack(Axis(0), &pts1).unwrap(), stack(Axis(0), &pts2).unwrap(),);
}

fn nn_test_random<D: 'static + Distance<f64> + Clone>(builder: &CommonNearestNeighbour, dist_fn: D) {
	let mut rng = Xoshiro256Plus::seed_from_u64(40);
	let n_points = 50000;
	let n_features = 3;
	let points = Array2::random_using((n_points, n_features), Uniform::new(-50., 50.), &mut rng);

	let linear = LinearSearch::new().from_batch(&points, dist_fn.clone()).unwrap();

	let nn = builder.from_batch(&points, dist_fn).unwrap();

	let pt = arr1(&[0., 0., 0.]);
	assert_eq_queries(nn.k_nearest(pt.view(), 5).unwrap(), linear.k_nearest(pt.view(), 5).unwrap());
	assert_eq_queries(
	...
	assert_eq_queries(nn.k_nearest(pt.view(), 30).unwrap(), linear.k_nearest(pt.view(), 30).unwrap());
	assert_eq_queries(
