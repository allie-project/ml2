...
fn partition<F: Float>(mut points: Vec<(Point<F>, usize)>) -> (Vec<(Point<F>, usize)>, Point<F>, Vec<(Point<F>, usize)>) {
	...
	let max_spread_dim = (0..points[0].0.len())
		.map(|dim| {
			...
			let (max, min) = points
				.iter()
				.map(|p| p.0[dim])
				.fold((F::neg_infinity(), F::infinity()), |(a, b), c| (F::max(a, c), F::min(b, c)));
	...
	let median = order_stat::kth_by(&mut points, mid, |p1, p2| p1.0[max_spread_dim].partial_cmp(&p2.0[max_spread_dim]).expect("NaN in data"))
		.0
		.reborrow();

	let (mut left, mut right): (Vec<_>, Vec<_>) = points.into_iter().partition(|pt| pt.0[max_spread_dim] < median[max_spread_dim]);
...
fn calc_radius<'a, F: Float, D: Distance<F>>(points: impl Iterator<Item = Point<'a, F>>, center: Point<F>, dist_fn: &D) -> F {
	let r_rad = points
...
impl<'a, F: Float> BallTreeInner<'a, F> {
	fn new<D: Distance<F>>(points: Vec<(Point<'a, F>, usize)>, leaf_size: usize, dist_fn: &D) -> Self {
		if points.len() <= leaf_size {
			...
			if let Some(dim) = points.first().map(|p| p.0.len()) {
				...
				let radius = calc_radius(points.iter().map(|p| p.0.reborrow()), center.view(), dist_fn);
				BallTreeInner::Leaf { center, radius, points }
			} else {
			...
			let radius = calc_radius(aps.iter().chain(bps.iter()).map(|p| p.0.reborrow()), center, dist_fn);
...
impl<'a, F: Float, D: Distance<F>> BallTreeIndex<'a, F, D> {
	...
	pub fn new<DT: Data<Elem = F>>(batch: &'a ArrayBase<DT, Ix2>, leaf_size: usize, dist_fn: D) -> Result<Self, BuildError> {
		...
		if leaf_size == 0 {
			...
			let points: Vec<_> = batch.rows().into_iter().enumerate().map(|(i, pt)| (pt, i)).collect();
			Ok(BallTreeIndex {
	...
	fn nn_helper<'b>(&self, point: Point<'b, F>, k: usize, max_radius: F) -> Result<Vec<(Point<F>, usize)>, NnError> {
		if self.dim != point.len() {
			...
			queue.push(MinHeapElem::new(self.tree.rdistance(point, &self.dist_fn), &self.tree));

			while let Some(MinHeapElem { dist: Reverse(dist), elem }) = queue.pop() {
...
impl<'a, F: Float, D: Distance<F>> NearestNeighbourIndex<F> for BallTreeIndex<'a, F, D> {
	fn k_nearest<'b>(&self, point: Point<'b, F>, k: usize) -> Result<Vec<(Point<F>, usize)>, NnError> {
		self.nn_helper(point, k, F::infinity())
	}

	fn within_range<'b>(&self, point: Point<'b, F>, range: F) -> Result<Vec<(Point<F>, usize)>, NnError> {
...
/// [`BallTreeIndex`](crate::nn::BallTreeIndex).
///
...
impl NearestNeighbour for BallTree {
	fn from_batch_with_leaf_size<'a, F: Float, DT: Data<Elem = F>, D: 'a + Distance<F>>(
		...
		BallTreeIndex::new(batch, leaf_size, dist_fn).map(|v| Box::new(v) as Box<dyn NearestNeighbourIndex<F>>)
	}
}
...
mod test {
	...
	fn assert_partition(input: Array2<f64>, exp_left: Array2<f64>, exp_med: Array1<f64>, exp_right: Array2<f64>, exp_rad: f64) {
		...
		assert_abs_diff_eq!(calc_radius(vec.iter().map(|(p, _)| p.reborrow()), mid, &L2Dist), exp_rad);
	}

	#[test]
	fn partition_test() {
		// partition 2 elements
		assert_partition(arr2(&[[0.0, 1.0], [2.0, 3.0]]), arr2(&[[0.0, 1.0]]), arr1(&[2.0, 3.0]), arr2(&[[2.0, 3.0]]), 8.0f64.sqrt());
		assert_partition(arr2(&[[2.0, 3.0], [0.0, 1.0]]), arr2(&[[0.0, 1.0]]), arr1(&[2.0, 3.0]), arr2(&[[2.0, 3.0]]), 8.0f64.sqrt());

		// Partition along the dimension with highest spread
		assert_partition(arr2(&[[0.3, 5.0], [4.5, 7.0], [8.1, 1.5]]), arr2(&[[0.3, 5.0]]), arr1(&[4.5, 7.0]), arr2(&[[4.5, 7.0], [8.1, 1.5]]), 43.21f64.sqrt());
