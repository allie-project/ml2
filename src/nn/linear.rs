...
use super::{distance::Distance, heap_elem::MinHeapElem, BuildError, NearestNeighbour, NearestNeighbourIndex, NnError, Point};
...
impl<'a, F: Float, D: Distance<F>> LinearSearchIndex<'a, F, D> {
	...
	pub fn new<DT: Data<Elem = F>>(batch: &'a ArrayBase<DT, Ix2>, dist_fn: D) -> Result<Self, BuildError> {
		if batch.ncols() == 0 {
...
impl<'a, F: Float, D: Distance<F>> NearestNeighbourIndex<F> for LinearSearchIndex<'a, F, D> {
	fn k_nearest<'b>(&self, point: Point<'b, F>, k: usize) -> Result<Vec<(Point<F>, usize)>, NnError> {
		if self.0.ncols() != point.len() {
			...
			Ok((0..k.min(heap.len())).map(|_| heap.pop().unwrap().elem).collect())
		}
	}
...
impl NearestNeighbour for LinearSearch {
	fn from_batch_with_leaf_size<'a, F: Float, DT: Data<Elem = F>, D: 'a + Distance<F>>(
		...
		LinearSearchIndex::new(batch, dist_fn).map(|v| Box::new(v) as Box<dyn NearestNeighbourIndex<F>>)
	}
}
