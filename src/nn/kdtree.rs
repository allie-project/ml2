...
use super::{distance::Distance, BuildError, NearestNeighbour, NearestNeighbourIndex, NnError, Point};
use crate::core::Float;

/// Spatial indexing structure created by [`KdTree`](struct.KdTree.html).
#[derive(Debug)]
pub struct KdTreeIndex<'a, F: Float, D: Distance<F>>(kdtree::KdTree<F, (Point<'a, F>, usize), &'a [F]>, D);
	...
	pub fn new<DT: Data<Elem = F>>(batch: &'a ArrayBase<DT, Ix2>, leaf_size: usize, dist_fn: D) -> Result<Self, BuildError> {
		if leaf_size == 0 {
			...
			for (i, point) in batch.rows().into_iter().enumerate() {
				tree.add(point.to_slice().expect("views should be contiguous"), (point, i)).unwrap();
			}
...
impl<'a, F: Float, D: Distance<F>> NearestNeighbourIndex<F> for KdTreeIndex<'a, F, D> {
	fn k_nearest<'b>(&self, point: Point<'b, F>, k: usize) -> Result<Vec<(Point<F>, usize)>, NnError> {
		Ok(self
			.0
			.nearest(point.to_slice().expect("views should be contiguous"), k, &|a, b| self.1.rdistance(aview1(a), aview1(b)))?
			.into_iter()
	...
	fn within_range<'b>(&self, point: Point<'b, F>, range: F) -> Result<Vec<(Point<F>, usize)>, NnError> {
		...
		Ok(self
			.0
			.within(point.to_slice().expect("views should be contiguous"), range, &|a, b| self.1.rdistance(aview1(a), aview1(b)))?
			.into_iter()
			.map(|(_, (pt, pos))| (pt.reborrow(), *pos))
			.collect())
	}
...
impl NearestNeighbour for KdTree {
	fn from_batch_with_leaf_size<'a, F: Float, DT: Data<Elem = F>, D: 'a + Distance<F>>(
		...
		KdTreeIndex::new(batch, leaf_size, dist_fn).map(|v| Box::new(v) as Box<dyn NearestNeighbourIndex<F>>)
	}
}
