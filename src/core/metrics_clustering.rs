...
use crate::core::dataset::{AsSingleTargets, DatasetBase, Label, Labels, Records};
...
pub trait SilhouetteScore<F> {
	...
	///
	/// Given a clustered dataset, the silhouette score for each sample is computed as the relative difference between
	/// the average distance of the sample to other samples in the same cluster and the minimum average distance of the
	/// sample to samples in another cluster. This value goes from -1 to +1 when the point is respectively closer (in
	/// average) to points in another cluster and to points in its own cluster.
	///
...
impl<F: Float> DistanceCount<F> {
	...
	pub fn new(count: usize) -> DistanceCount<F> {
		DistanceCount { total_distance: F::zero(), count }
	}
	...
	/// Adds the distance of `other_sample` from `eval_sample` to the total distance of `eval_sample` from the current
	/// cluster.
		...
		let score = self
			...
			.map(|sample| {
				...
				for other in self.sample_iter() {
					labels.get_mut(other.1.into_scalar()).unwrap().add_point(sample.0, other.0);
				}
				...
				if a_x >= b_x { (b_x - a_x) / a_x } else { (b_x - a_x) / b_x }
			})
			.sum::<F>();
...
mod tests {
	...
	use crate::core::metrics_clustering::SilhouetteScore;
	...
	fn test_silhouette_score() {
		...
		let records = concatenate![Axis(0), Array::linspace(0f64, 1f64, 10), Array::linspace(10000f64, 10001f64, 10)].insert_axis(Axis(1));
		...
		.insert_axis(Axis(1));
		let records = concatenate![Axis(1), records, records];
		let targets = concatenate![Axis(0), Array1::from_elem(5, 0), Array1::from_elem(5, 1), Array1::from_elem(5, 0), Array1::from_elem(5, 1)];
		let dataset: Dataset<_, _, Ix1> = (records, targets).into();
		let score = dataset.silhouette_score().unwrap();
		assert!(score < 0f64);
