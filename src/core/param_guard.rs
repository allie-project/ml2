...
where
	P::Checked: Fit<R, T, E>,
	E: Error + From<crate::core::error::Error> + From<P::Error>
{
	type Object = <<P as ParamGuard>::Checked as Fit<R, T, E>>::Object;

	fn fit(&self, dataset: &crate::core::DatasetBase<R, T>) -> Result<Self::Object, E> {
	...
	E: Error + From<crate::core::error::Error> + From<P::Error>
{
	...
	fn fit_with(&self, model: Self::ObjectIn, dataset: &'a crate::core::DatasetBase<R, T>) -> Result<Self::ObjectOut, E> {
		let checked = self.check_ref()?;
