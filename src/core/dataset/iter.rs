...
impl<'a, 'b: 'a, F, L, I: TargetDim> Iterator for Iter<'a, 'b, F, L, I> {
	...
	fn next(&mut self) -> Option<Self::Item> {
		...
		Some((records.index_axis_move(Axis(0), self.idx - 1), targets.index_axis_move(Axis(0), self.idx - 1)))
	}
}
...
impl<'a, 'b: 'a, R: Records, T> DatasetIter<'a, 'b, R, T> {
	pub fn new(dataset: &'b DatasetBase<R, T>, target_or_feature: bool) -> DatasetIter<'a, 'b, R, T> {
		DatasetIter {
			dataset,
			idx: 0,
			target_or_feature,
	...
	T: AsTargets<Elem = E> + FromTargetArray<'b>
{
	...
	fn next(&mut self) -> Option<Self::Item> {
		...
		let (mut records, mut targets) = (self.records.reborrow(), self.targets.as_targets().reborrow());
