...
use ndarray::{Array, Array1, Array2, ArrayBase, ArrayView, ArrayViewMut, Axis, CowArray, Data, DataMut, Dimension, Ix1, Ix2, Ix3, OwnedRepr, ViewRepr};

use super::{
	AsMultiTargets, AsMultiTargetsMut, AsProbabilities, AsSingleTargets, AsSingleTargetsMut, AsTargets, AsTargetsMut, CountedTargets, DatasetBase,
	FromTargetArray, Label, Labels, Pr, TargetDim
};
...
/// An NdArray with discrete labels can act as labels
	...
	T: AsTargets<Elem = L>
{
	...
	pub fn with_labels(&self, labels: &[L]) -> DatasetBase<Array2<F>, CountedTargets<L, Array<L, T::Ix>>> {
		...
		for (i, (r, t)) in self.records().rows().into_iter().zip(targets.axis_iter(Axis(0)).into_iter()).enumerate() {
		...
		let records = Array2::from_shape_vec(self.records.raw_dim().nsamples(nsamples), records_arr).unwrap();
		let targets = Array::from_shape_vec(self.targets.as_targets().raw_dim().nsamples(nsamples), targets_arr).unwrap();

		let targets = CountedTargets { targets, labels: map };
