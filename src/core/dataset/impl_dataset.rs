...
use super::{
	...
	AsSingleTargets, AsTargets, AsTargetsMut, CountedTargets, Dataset, DatasetBase, DatasetView, Float, FromTargetArray, Label, Labels, Records, Result,
	TargetDim
};
use crate::core::traits::Fit;
...
impl<R: Records, S> DatasetBase<R, S> {
	...
	/// Returns optional weights.
	pub fn weights(&self) -> Option<&[f32]> {
		if !self.weights.is_empty() { Some(self.weights.as_slice().unwrap()) } else { None }
	}
	...
	pub fn feature_names(&self) -> Vec<String> {
		if !self.feature_names.is_empty() {
			...
			(0..self.records.nfeatures()).map(|idx| format!("feature-{}", idx)).collect()
		}
	}
...
impl<L, R: Records, T: AsTargets<Elem = L>> DatasetBase<R, T> {
	...
	pub fn ntargets(&self) -> usize {
		if T::Ix::NDIM.unwrap() == 1 { 1 } else { self.targets.as_targets().len_of(Axis(1)) }
	}
}
	...
	T: AsTargets<Elem = L>
{
	...
	///
	/// This function creates an iterator which produces tuples of data points and target values. The
	/// iterator runs once for each data point and, while doing so, holds a reference to the owned
	/// dataset.
	///
	/// For multi-target datasets, the yielded target value is an `ArrayView1` consisting of the
	/// different targets. For single-target datasets, the target value is an `ArrayView0` containing
	/// the single target.
	///
	/// # Example
	/// ```
...
impl<F, E, D, S, I: TargetDim> From<(ArrayBase<D, Ix2>, ArrayBase<S, I>)> for DatasetBase<ArrayBase<D, Ix2>, ArrayBase<S, I>>
where
	D: Data<Elem = F>,
	S: Data<Elem = E>
{
	...
	T::Owned: AsTargets
{
	...
	/// # Returns
	///
	/// An infinite Iterator yielding at each step of a new bootstrapped dataset.
	pub fn bootstrap<R: Rng>(
		...
		std::iter::repeat(()).map(move |_| {
			...
			let indices = (0..sample_feature_size.0).map(|_| rng.gen_range(0..self.nsamples())).collect::<Vec<_>>();

			let records = self.records().select(Axis(0), &indices);
			let targets = T::new_targets(self.as_targets().select(Axis(0), &indices));

			let indices = (0..sample_feature_size.1).map(|_| rng.gen_range(0..self.nfeatures())).collect::<Vec<_>>();
	...
	pub fn bootstrap_samples<R: Rng>(
		...
		std::iter::repeat(()).map(move |_| {
			// sample with replacement
			let indices = (0..num_samples).map(|_| rng.gen_range(0..self.nsamples())).collect::<Vec<_>>();
	...
	pub fn bootstrap_features<R: Rng>(
		...
		std::iter::repeat(()).map(move |_| {
			...
			let indices = (0..num_features).map(|_| rng.gen_range(0..self.nfeatures())).collect::<Vec<_>>();
	...
	pub fn fold(&self, k: usize) -> Vec<(DatasetBase<Array2<F>, T::Owned>, DatasetBase<Array2<F>, T::Owned>)> {
		...
		let mut records_chunks: Vec<_> = self.records.axis_chunks_iter(Axis(0), fold_size).collect();
		...
		for i in 0..k {
			...
			res.push((
				...
				// validation
				DatasetBase::new(records_chunks[0].into_owned(), T::new_targets(targets_chunks[0].clone().into_owned()))
			));
	...
	pub fn to_owned(&self) -> DatasetBase<Array2<F>, T::Owned> {
		DatasetBase::new(self.records().to_owned(), T::new_targets(self.as_targets().to_owned()))
	}
}
	...
	S: DataMut<Elem = E>
{
			...
			for i in 0..k {
...

				{
					let train = DatasetBase::new(
						ArrayView2::from_shape((samples_count - fold_size, features), records_sl.split_at(fold_size * features).1).unwrap(),
						ArrayView::from_shape(tshape.clone().nsamples(samples_count - fold_size), targets_sl.split_at(fold_size * targets).1).unwrap()
					);
	...
	/// ```
	pub fn cross_validate<O, ER, M, FACC, C>(&'a mut self, k: usize, parameters: &[M], eval: C) -> std::result::Result<Array<FACC, I>, ER>
	where
		ER: std::error::Error + std::convert::From<crate::core::error::Error>,
		M: for<'c> Fit<ArrayView2<'c, F>, ArrayView<'c, E, I>, ER, Object = O>,
		O: for<'d> PredictInplace<ArrayView2<'a, F>, Array<E, I>>,
		FACC: Float,
		C: Fn(&Array<E, I>, &ArrayView<E, I>) -> std::result::Result<Array<FACC, I::Smaller>, crate::core::error::Error>
	{
		let mut evaluations = Array::from_elem(self.targets.raw_dim().nsamples(parameters.len()), FACC::zero());
		let folds_evaluations: std::result::Result<Vec<_>, ER> = self
			.iter_fold(k, |train| {
				let fit_result: std::result::Result<Vec<_>, ER> = parameters.iter().map(|p| p.fit(train)).collect();
				fit_result
			})
			.map(|(models, valid)| {
				...
				let mut eval_predictions = Array::from_elem(targets.raw_dim().nsamples(models.len()), FACC::zero());
				for (i, model) in models.iter().enumerate() {
					...
					eval_predictions.index_axis_mut(Axis(0), i).add_assign(&eval_pred);
				}
				Ok(eval_predictions)
			})
			.collect();
	...
	S: DataMut<Elem = E>
{
	...
	/// more details.
	pub fn cross_validate_single<O, ER, M, FACC, C>(&'a mut self, k: usize, parameters: &[M], eval: C) -> std::result::Result<Array1<FACC>, ER>
	where
		ER: std::error::Error + std::convert::From<crate::core::error::Error>,
		M: for<'c> Fit<ArrayView2<'c, F>, ArrayView1<'c, E>, ER, Object = O>,
		O: for<'d> PredictInplace<ArrayView2<'a, F>, Array1<E>>,
		FACC: Float,
		C: Fn(&Array1<E>, &ArrayView1<E>) -> std::result::Result<FACC, crate::core::error::Error>
	{
...
impl<F, E, I: TargetDim> Dataset<F, E, I> {
	...
	/// two new Datasets will have respectively `n * ratio` and `n - (n*ratio)` samples. For example,
	/// a ratio of `0.9` allocates 90% to the first chunks and 10% to the second. This is often used
	/// in training & validation splitting procedures.
	///
	/// ### Parameters
	///
	...
	pub fn split_with_ratio(mut self, ratio: f32) -> (Self, Self) {
		assert!(self.records.is_standard_layout(), "records not in row-major layout");
		assert!(self.targets.is_standard_layout(), "targets not in row-major layout");
