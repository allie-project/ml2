//! Implements Platt calibration with Newton method.
//!
...
use crate::core::traits::{FitWith, Predict, PredictInplace};
...
impl<F: Float, O> PlattParams<F, O> {
	...
	///
	/// Newton's method is an iterative optimization process, which uses the first and second
	...
	/// Set the minimum step size in the line search.
	///
	/// After estimating the Hessian matrix, a line search is performed to find the optimal step
	/// size in each optimization step. In each attempt, the step size is halved until this threshold
	...
	/// function sets the amount of regularization.
...
impl<F: Float, O> ParamGuard for PlattParams<F, O> {
	...
	fn check_ref(&self) -> Result<&Self::Checked, PlattError> {
		if self.0.maxiter == 0 {
			...
			Err(PlattError::MinStepNegative(self.0.minstep.to_f32().unwrap()))
...
impl<'a, F: Float, O: 'a> FitWith<'a, Array2<F>, Array1<bool>, PlattError> for PlattValidParams<F, O>
where
	O: PredictInplace<Array2<F>, Array1<F>>
{
	...
	/// probability targets.
	fn fit_with(&self, obj: O, ds: &DatasetBase<Array2<F>, Array1<bool>>) -> Result<Self::ObjectOut, PlattError> {
...
pub fn platt_newton_method<'a, F: Float, O>(
	...
	let reg_values = reg_values.into_iter().map(|x| F::from(*x).unwrap()).collect::<Vec<_>>();
	...
	for _ in 0..params.maxiter {
		...
		for (v, t) in reg_values.iter().zip(target_values.iter()) {
			...
			let (p, q) = if f_apb >= F::zero() {
				((-f_apb).exp() / (F::one() + (-f_apb).exp()), F::one() / (F::one() + (-f_apb).exp()))
			} else {
				(F::one() / (F::one() + f_apb.exp()), f_apb.exp() / (F::one() + f_apb.exp()))
			};
		...
		if g1.abs() < F::from(1e-5 * reg_values.len() as f32).unwrap() && g2.abs() < F::from(1e-5 * reg_values.len() as f32).unwrap() {
			break;
		}
...
mod tests {
	...
	fn generate_dummy_values<F: Float, R: Rng>(a: F, b: F, n: usize, rng: &mut R) -> (Array1<F>, Array1<bool>) {
		...
		let reg_values = prob_values.iter().map(|x| (F::one() - *x) / *x).map(|x| (x.ln() - b) / a).collect();
		// roll decision according to probability
		let decisions = prob_values.iter().map(|x| rng.gen_bool(x.to_f64().unwrap())).collect();
