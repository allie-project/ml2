...
impl<L: Clone + Default, F, D: Data<Elem = F>> PredictInplace<ArrayBase<D, Ix2>, Array1<L>> for MultiClassModel<ArrayBase<D, Ix2>, L> {
	fn predict_inplace(&self, arr: &ArrayBase<D, Ix2>, y: &mut Array1<L>) {
		assert_eq!(arr.nrows(), y.len(), "The number of data points must match the number of output targets.");
...
impl<F, D: Data<Elem = F>, L, P: PredictInplace<ArrayBase<D, Ix2>, Array1<Pr>> + 'static> FromIterator<(L, P)> for MultiClassModel<ArrayBase<D, Ix2>, L> {
	fn from_iter<I: IntoIterator<Item = (L, P)>>(iter: I) -> Self {
		let models = iter
			.into_iter()
			.map(|(l, x)| (l, Box::new(x) as Box<dyn PredictInplace<ArrayBase<D, Ix2>, Array1<Pr>>>))
			.collect();
...
mod tests {
	...
	#[test]
	fn choose_correct() {
		let model = vec![(0, DummyModel { on_even: false }), (1, DummyModel { on_even: true })]
			.into_iter()
