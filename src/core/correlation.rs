...
impl<F: Float> PearsonCorrelation<F> {
	/// Calculate the Pearson Correlation Coefficients and optional p-values from dataset.
	///
	/// The PCC describes the linear correlation between two variables. It is the covariance divided by
	/// the product of the standard deviations, therefore essentially a normalised measurement of the
	/// covariance and in range (-1, 1). A negative coefficient indicates a negative correlation
	...
	///
	/// This implementation estimates the p value with the permutation test. In null-hypothesis,
	...
	pub fn get_p_values(&self) -> Option<&Array1<F>> {
		if self.p_values.is_empty() { None } else { Some(&self.p_values) }
	}
}
...
/// Display the Pearson's Correlation Coefficients as an upper triangular matrix.
///
...
mod tests {
	...
	fn perfectly_correlated() {
		...
		let corr = DatasetBase::from(concatenate![Axis(1), data, data_proj]).pearson_correlation_with_p_value(100);
