...
pub trait SingleTargetRegression<F: Float, T: AsSingleTargets<Elem = F>>: AsSingleTargets<Elem = F> {
	...
	fn median_absolute_error(&self, compare_to: &T) -> Result<F> {
		let mut abs_error = self.as_single_targets().sub(&compare_to.as_single_targets()).mapv(|x| x.abs()).to_vec();
	...
	/// R-squared coefficient is the proportion of the variance in the dependent variable that is predictable from the
	/// independent variable `r2 = 1 - sum((pred_i - y_i)^2)/sum((mean_y - y_i)^2)`. If the mean is of `compare_to`,
	/// then the denominator should compare `compare_to` and the mean, and not self and the mean.
	fn r2(&self, compare_to: &T) -> Result<F> {
		let single_target_compare_to = compare_to.as_single_targets();
		let mean = single_target_compare_to.mean().ok_or(Error::NotEnoughSamples)?;

		Ok(F::one()
			- self.as_single_targets().sub(&single_target_compare_to).mapv(|x| x * x).sum()
				/ (single_target_compare_to.mapv(|x| (x - mean) * (x - mean)).sum() + F::cast(1e-10)))
	}
	...
	fn explained_variance(&self, compare_to: &T) -> Result<F> {
		...
		Ok(F::one() - (diff.mapv(|x| x * x).sum() - mean_error) / (single_target_compare_to.mapv(|x| (x - mean) * (x - mean)).sum() + F::cast(1e-10)))
	}
}

impl<F: Float, D: Data<Elem = F>, T: AsSingleTargets<Elem = F>> SingleTargetRegression<F, T> for ArrayBase<D, Ix1> {}

impl<F: Float, T: AsSingleTargets<Elem = F>, T2: AsSingleTargets<Elem = F>, D: Data<Elem = F>> SingleTargetRegression<F, T2>
...
impl<F: Float, D: Data<Elem = F>, T: AsMultiTargets<Elem = F>> MultiTargetRegression<F, T> for ArrayBase<D, Ix2> {}

impl<F: Float, T: AsMultiTargets<Elem = F>, T2: AsMultiTargets<Elem = F>, D: Data<Elem = F>> MultiTargetRegression<F, T2>
	for DatasetBase<ArrayBase<D, Ix2>, T>
{
}
...
mod tests {
	...
	use crate::core::dataset::DatasetBase;
	...
	fn test_mean_absolute_error_for_single_targets() {
		...
		let abs_err_from_ds = prediction.mean_absolute_error(st_dataset.targets()).unwrap();
	...
	fn test_mean_squared_log_error_for_single_targets() {
		...
		let st_dataset: DatasetBase<_, _> = (records.view(), targets).into();
		let prediction = array![0.1, 0.3, 0.2, 0.5, 0.7];
		let abs_err_from_arr1 = prediction.mean_squared_log_error(st_dataset.targets()).unwrap();
		let prediction: DatasetBase<_, _> = (records.view(), prediction).into();
		let abs_err_from_ds = prediction.mean_squared_log_error(st_dataset.targets()).unwrap();
	...
	fn test_median_absolute_error_for_single_targets() {
		...
		let abs_err_from_arr1 = prediction.median_absolute_error(st_dataset.targets()).unwrap();
		let prediction: DatasetBase<_, _> = (records.view(), prediction).into();
		let abs_err_from_ds = prediction.median_absolute_error(st_dataset.targets()).unwrap();
