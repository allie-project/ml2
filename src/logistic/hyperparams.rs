...
impl<F: Float, D: Dimension> Default for LogisticRegressionParams<F, D> {
	fn default() -> Self {
		LogisticRegressionParams(LogisticRegressionValidParams {
			alpha: F::cast(1.0),
			fit_intercept: true,
			max_iterations: 100,
...
impl<F: Float, D: Dimension> LogisticRegressionParams<F, D> {
	...
	/// Set the regularization parameter `alpha` used for L2 regularization. Defaults to `1.0`.
