...
use crate::nn::{distance::L2Dist, NearestNeighbour};

/// Create sparse adjacency matrix from dense dataset
pub fn adjacency_matrix<F: Float, DT: Data<Elem = F>, N: NearestNeighbour>(dataset: &ArrayBase<DT, Ix2>, k: usize, nn_algo: &N) -> CsMat<F> {
	...
	let nn = nn_algo.from_batch(dataset, L2Dist).expect("Unexpected nearest neighbour error");
...
mod tests {
	...
	fn adjacency_matrix_test_2() {
		...
		let input_mat = vec![0., 0., 3.1, 3.1, 1., 1., 2.1, 1.1, 2., 2., 1.1, 1.1, 3., 3., 0.1, 0.1];
	...
	fn sparse_panics_on_0_neighbours() {
		let input_mat = [[[0., 0.], [0.1, 0.1]], [[1., 1.], [1.1, 1.1]], [[2., 2.], [2.1, 2.1]], [[3., 3.], [3.1, 3.1]]]
			.concat()
			.concat();
		let input_arr = Array2::from_shape_vec((8, 2), input_mat).unwrap();
