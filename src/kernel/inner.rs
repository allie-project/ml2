...
impl<F: Float, D: Data<Elem = F>> Inner for ArrayBase<D, Ix2> {
	...
	fn to_upper_triangle(&self) -> Vec<F> {
		self.indexed_iter().filter(|((row, col), _)| col > row).map(|(_, val)| *val).collect()
	}
...
impl<F: Float> Inner for CsMat<F> {
	...
	fn column(&self, i: usize) -> Vec<F> {
		(0..self.size()).map(|j| *self.get(j, i).unwrap_or(&F::neg_zero())).collect::<Vec<_>>()
	}
	fn to_upper_triangle(&self) -> Vec<F> {
		let mat = self.to_dense();
		mat.indexed_iter().filter(|((row, col), _)| col > row).map(|(_, val)| *val).collect()
	}
