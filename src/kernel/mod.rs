...
use crate::nn::CommonNearestNeighbour;
...
impl<'a, F: Float> Kernel<F> {
	pub fn new<N: NearestNeighbour>(dataset: ArrayView2<'a, F>, params: &KernelParams<F, N>) -> Kernel<F> {
		let inner = match params.kind {
			...
			KernelType::Sparse(k) => KernelInner::Sparse(sparse_from_fn(&dataset, k, &params.method, &params.nn_algo))
		};

		Kernel { inner, method: params.method.clone() }
	}
	...
	pub fn distance(&self, a: ArrayView1<F>, b: ArrayView1<F>) -> F {
		match *self {
			KernelMethod::Gaussian(eps) => {
				let distance = a.iter().zip(b.iter()).map(|(x, y)| (*x - *y) * (*x - *y)).sum::<F>();
...
impl<'a, F: Float, N: NearestNeighbour> Transformer<ArrayView2<'a, F>, Kernel<F>> for KernelParams<F, N> {
	/// Builds a kernel from a view of the input data.
	///
	/// ## Parameters
	///
...
impl<'a, F: Float, L: 'a, T: AsTargets<Elem = L> + FromTargetArray<'a>, N: NearestNeighbour>
	Transformer<&'a DatasetBase<Array2<F>, T>, DatasetBase<Kernel<F>, T::View>> for KernelParams<F, N>
{
	/// Builds a new Dataset with the kernel as the records and the same targets as the input one.
	///
	/// ## Parameters
...
impl<'a, 'b, F: Float, L: 'b, T: AsTargets<Elem = L> + FromTargetArray<'b>, N: NearestNeighbour>
	Transformer<&'b DatasetBase<ArrayView2<'a, F>, T>, DatasetBase<Kernel<F>, T::View>> for KernelParams<F, N>
{
	/// Builds a new Dataset with the kernel as the records and the same targets as the input one.
	///
	/// ## Parameters
...
fn dense_from_fn<F: Float, D: Data<Elem = F>>(dataset: &ArrayBase<D, Ix2>, method: &KernelMethod<F>) -> Array2<F> {
...
fn sparse_from_fn<F: Float, D: Data<Elem = F>, N: NearestNeighbour>(dataset: &ArrayBase<D, Ix2>, k: usize, method: &KernelMethod<F>, nn_algo: &N) -> CsMat<F> {
...
mod tests {
	...
	fn sparse_from_fn_test() {
		...
		let input_mat = vec![0., 0., 0.1, 0.1, 1., 1., 1.1, 1.1, 2., 2., 2.1, 2.1, 3., 3., 3.1, 3.1];
	...
	fn dense_from_fn_test() {
		...
		for i in 0..8 {
			for j in 0..8 {
				assert!((similarity_matrix.row(i)[j] - method.distance(input_arr.row(i), input_arr.row(j))).abs() <= f64::EPSILON);
			}
		}
	}
	...
	fn test_kernel_dot() {
		...
		// sparse kernel dot
		let mul_mat = sparse_from_fn(&input_arr, 3, &KernelMethod::Linear, &KdTree).mul(&to_multiply.view());
		let kernel = KernelView::params()
	...
	fn test_kernel_upper_triangle() {
		...
		let input_arr = ndarray::concatenate(Axis(0), &[input_arr_1.view(), input_arr_2.view()]).unwrap();
	...
	fn test_kernel_sum() {
		...
		// sparse kernel sum
		let cols_sum = sparse_from_fn(&input_arr, 3, &method, &BallTree).to_dense().sum_axis(Axis(1));
		let kernel = KernelView::params()
			.kind(KernelType::Sparse(3))
			.method(method)
			.transform(input_arr.view());
	...
	fn test_kernel_diag() {
		...
		assert!(arrays_almost_equal(input_diagonal.view(), kers_diagonal.view()));
	...
	fn check_kernel_from_dataset_type<'a, L: 'a, T: AsTargets<Elem = L> + FromTargetArray<'a>>(input: &'a DatasetBase<Array2<f64>, T>, k_type: KernelType) {
		let methods = vec![KernelMethod::Linear, KernelMethod::Gaussian(0.1), KernelMethod::Polynomial(1., 2.)];
		for method in methods {
			let kernel_ref = Kernel::new(input.records().view(), &Kernel::params_with_nn(KdTree).method(method.clone()).kind(k_type.clone()));
			let kernel_tr = Kernel::params().kind(k_type.clone()).method(method.clone()).transform(input);
	...
	fn check_kernel_from_dataset_view_type<'a, L: 'a, T: AsTargets<Elem = L> + FromTargetArray<'a>>(
		input: &'a DatasetBase<ArrayView2<'a, f64>, T>,
		k_type: KernelType
	) {
		let methods = vec![KernelMethod::Linear, KernelMethod::Gaussian(0.1), KernelMethod::Polynomial(1., 2.)];
		for method in methods {
			let kernel_ref = Kernel::new(*input.records(), &Kernel::params_with_nn(KdTree).method(method.clone()).kind(k_type.clone()));
			let kernel_tr = Kernel::params().kind(k_type.clone()).method(method.clone()).transform(input);
			match (&kernel_ref.inner, &kernel_tr.records().inner) {
				(KernelInner::Dense(m1), KernelInner::Dense(m2)) => {
					assert!(kernels_almost_equal(m1, m2))
	...
	fn check_kernel_from_array2_type(input: &Array2<f64>, k_type: KernelType) {
		let methods = vec![KernelMethod::Linear, KernelMethod::Gaussian(0.1), KernelMethod::Polynomial(1., 2.)];
		for method in methods {
			let kernel_ref = Kernel::new(input.view(), &Kernel::params_with_nn(KdTree).method(method.clone()).kind(k_type.clone()));
			let kernel_tr = Kernel::params().kind(k_type.clone()).method(method.clone()).transform(input.view());
	...
	fn check_kernel_from_array_view_2_type(input: ArrayView2<f64>, k_type: KernelType) {
		let methods = vec![KernelMethod::Linear, KernelMethod::Gaussian(0.1), KernelMethod::Polynomial(1., 2.)];
		for method in methods {
			let kernel_ref = Kernel::new(input, &Kernel::params_with_nn(KdTree).method(method.clone()).kind(k_type.clone()));
			let kernel_tr = Kernel::params().kind(k_type.clone()).method(method.clone()).transform(input);
			match (&kernel_ref.inner, &kernel_tr.inner) {
				(KernelInner::Dense(m1), KernelInner::Dense(m2)) => {
					assert!(kernels_almost_equal(m1, m2))
	...
	fn matrices_almost_equal(reference: ArrayView2<f64>, transformed: ArrayView2<f64>) -> bool {
		for (ref_row, tr_row) in reference.axis_iter(Axis(0)).zip(transformed.axis_iter(Axis(0))) {
