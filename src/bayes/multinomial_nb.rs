...
use super::base_nb::{filter, NaiveBayes, NaiveBayesValidParams};
use super::error::{NaiveBayesError, Result};
use super::hyperparams::{MultinomialNbParams, MultinomialNbValidParams};
use crate::core::dataset::{AsSingleTargets, DatasetBase, Labels};
use crate::core::traits::{Fit, FitWith, PredictInplace};
...
impl<'a, F, L, D, T> FitWith<'a, ArrayBase<D, Ix2>, T, NaiveBayesError> for MultinomialNbValidParams<F, L>
where
	F: Float,
	L: Label + 'a,
	D: Data<Elem = F>,
	T: AsSingleTargets<Elem = L> + Labels<Elem = L>
{
	...
	fn fit_with(&self, model_in: Self::ObjectIn, dataset: &DatasetBase<ArrayBase<D, Ix2>, T>) -> Result<Self::ObjectOut> {
		...
		let mut model = match model_in {
			Some(temp) => temp,
			None => MultinomialNb { class_info: HashMap::new() }
		};

		let yunique = dataset.labels();

		for class in yunique {
			// Filter for records that correspond to the current class
			let xclass = filter(x.view(), y.view(), &class);
			// Count the number of occurences of the class
			let nclass = xclass.nrows();

			// Compute the feature log probabilities and feature counts on the slice corresponding to the current class
			let mut class_info = model.class_info.entry(class).or_insert_with(MultinomialClassInfo::default);
			let (feature_log_prob, feature_count) = self.update_feature_log_prob(class_info, xclass.view());
			// Now update the total counts of each feature, feature log probabilities, and class count
		...
		// Update the priors
		let class_count_sum = model.class_info.values().map(|x| x.class_count).sum::<usize>();
	...
	F: Float
{
	...
	fn update_feature_log_prob(&self, info_old: &MultinomialClassInfo<F>, x_new: ArrayView2<F>) -> (Array1<F>, Array1<F>) {
		// Deconstruct old state
		let (count_old, feature_log_prob_old, feature_count_old) = (&info_old.class_count, &info_old.feature_log_prob, &info_old.feature_count);
		...
		let feature_count = if count_old > &0 { feature_count_old + feature_count_new } else { feature_count_new };
...
///
/// ```rust
/// use ml2::bayes::{MultinomialNbParams, MultinomialNbValidParams, Result};
...
/// let unchecked_params = MultinomialNbParams::new().alpha(1.0);
///
...
mod tests {
	...
	fn test_multinomial_nb() -> Result<()> {
		...
		expected.insert(&1usize, array![-0.82667857, -0.96020997, -1.09374136, -2.77258872, -4.85203026, -6.93147181]);

		expected.insert(&2usize, array![-2.77258872, -4.85203026, -6.93147181, -0.82667857, -0.96020997, -1.09374136]);
