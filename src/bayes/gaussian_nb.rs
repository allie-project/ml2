...
use super::hyperparams::{GaussianNbParams, GaussianNbValidParams};
...
impl<'a, F, L, D, T> FitWith<'a, ArrayBase<D, Ix2>, T, NaiveBayesError> for GaussianNbValidParams<F, L>
where
	F: Float,
	L: Label + 'a,
	D: Data<Elem = F>,
	T: AsSingleTargets<Elem = L> + Labels<Elem = L>
{
	...
	fn fit_with(&self, model_in: Self::ObjectIn, dataset: &DatasetBase<ArrayBase<D, Ix2>, T>) -> Result<Self::ObjectOut> {
		...
		let mut model = match model_in {
			Some(mut temp) => {
				temp.class_info.values_mut().for_each(|x| x.sigma -= epsilon);
				temp
			}
			None => GaussianNb { class_info: HashMap::new() }
		};
		...
		for class in yunique {
			...
			let mut class_info = model.class_info.entry(class).or_insert_with(GaussianClassInfo::default);
			// Compute the update of the gaussian mean and variance
		...
		// Add back the epsilon previously subtracted for numerical calculation stability
		model.class_info.values_mut().for_each(|x| x.sigma += epsilon);
	...
	F: Float
{
	...
	fn update_mean_variance(info_old: &GaussianClassInfo<F>, x_new: ArrayView2<F>) -> (Array1<F>, Array1<F>) {
...
/// let unchecked_params = GaussianNbParams::new().var_smoothing(1e-5);
///
	...
	L: Label + Ord
{
	...
	fn joint_log_likelihood(&self, x: ArrayView2<F>) -> HashMap<&L, Array1<F>> {
		...
		for (class, info) in self.class_info.iter() {
			...
			let mut nij = info.sigma.mapv(|x| F::cast(2. * std::f64::consts::PI) * x).mapv(|x| x.ln()).sum();
			nij = F::cast(-0.5) * nij;
...
mod tests {
	...
	fn test_gaussian_nb() -> Result<()> {
		...
		expected
			.insert(&1usize, array![-2.276946847943017, -1.5269468546930165, -2.276946847943017, -25.52694663869301, -38.27694652394301, -38.27694652394301]);
		expected
			.insert(&2usize, array![-38.27694652394301, -25.52694663869301, -38.27694652394301, -1.5269468546930165, -2.276946847943017, -2.276946847943017]);
